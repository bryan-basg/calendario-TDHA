...
async def test_auth_errors(client):
    ...
    response = await client.post(
        "/token", data={"username": "wrong@example.com", "password": "pwd"}
    )
...
async def test_validation_errors(client):
    ...
    resp = await client.post(
        "/events/",
        json={"title": "No Cat", "start_time": "2025-01-01T10:00:00"},
        headers=headers,
    )
    assert resp.status_code == 422
...
async def test_permission_errors(client):
    ...
    t1_resp = await client.post(
        "/tasks/", json={"title": "U1 Task", "energy_required": "low"}, headers=h1
    )
    ...
    c_resp = await client.post(
        "/categories/", json={"name": "Common", "color_hex": "#000"}, headers=h1
    )
...
async def test_additional_endpoints(client):
    ...
    t_resp = await client.post(
        "/tasks/",
        json={"title": "To Complete", "energy_required": "low"},
        headers=headers,
    )
    t_id = t_resp.json()["id"]
    ...
    # First create event
    c_resp = await client.post(
        "/categories/", json={"name": "Cat", "color_hex": "#000"}, headers=headers
    )
    cat_id = c_resp.json()["id"]
    ...
    resp = await client.put(
        f"/events/{e_id}", json={"category_id": 99999}, headers=headers
    )
