...
def test_category_management(db_session):
    ...
    cat = crud.create_category(
        db_session, schemas.CategoryCreate(name="Home", color_hex="#fff")
    )
...
def test_timeline_logic(db_session, test_user, test_category):
    ...
    # Fetch Timeline
    timeline = crud.get_timeline(
        db_session, test_user.id, now, now + timedelta(hours=24)
    )
...
def test_now_view(db_session, test_user):
    ...
    future_task = crud.create_user_task(
        db_session,
        schemas.TaskCreate(
            title="Future",
    ...
    current_task = crud.create_user_task(
        db_session,
        schemas.TaskCreate(
            title="Current",
            energy_required="high",
...
def test_task_suggestions(db_session, test_user):
    ...
    t_urgent = crud.create_user_task(
        db_session,
        schemas.TaskCreate(
            title="Urgent",
            energy_required="medium",
    ...
    # Tarea High Energy
    t_heavy = crud.create_user_task(
        db_session,
        schemas.TaskCreate(title="Heavy", energy_required="high"),
        test_user.id,
    )

    # Tarea Low Energy
    t_easy = crud.create_user_task(
        db_session,
        schemas.TaskCreate(title="Easy", energy_required="low"),
        test_user.id,
    )

    # Test Low Energy User -> Prefers Easy
    sug_low = crud.get_task_suggestions(
        db_session, test_user.id, models.EnergyLevel.low
    )
    # Debería priorizar Urgent primero (50 pts) y luego Easy (Low+Low=40 pts) vs Heavy (Low+High=-20)
    # NOTA: La implementación optimizada filtra Heavy tasks si el usuario está en Low energy y no es urgente.
    assert len(sug_low) == 2
    assert sug_low[0].id == t_urgent.id
    assert sug_low[1].id == t_easy.id

    # Test High Energy User -> Prefers Heavy (after urgent)
    sug_high = crud.get_task_suggestions(
        db_session, test_user.id, models.EnergyLevel.high
    )
...
def test_timeline_task_defaults(db_session, test_user):
    ...
    t = crud.create_user_task(
        db_session,
        schemas.TaskCreate(title="No End", energy_required="low", planned_start=now),
        test_user.id,
    )

    timeline = crud.get_timeline(
        db_session, test_user.id, now, now + timedelta(hours=1)
    )
    item = timeline[0]
    # Verifica que calculó 30 min por defecto
    expected_end = crud.ensure_utc(t.planned_start) + timedelta(minutes=30)
