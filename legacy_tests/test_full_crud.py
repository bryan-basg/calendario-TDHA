...
async def run_tests():
    ...
    async with httpx.AsyncClient(transport=transport, base_url="http://test") as client:
        ...
        # Login A
        resp = await client.post("/token", data={"username": email_a, "password": pwd_a})
        ...
        resp = await client.post("/token", data={"username": email_b, "password": pwd_b})
        assert resp.status_code == 200, f"User B login failed: {resp.text}"
        token_b = resp.json()["access_token"]
        headers_b = {"Authorization": f"Bearer {token_b}"}

        # ---------- CATEGORY ----------
        cat_resp = await client.post("/categories/", json={"name": "TestCat", "color_hex": "#123456"}, headers=headers_a)
        assert cat_resp.status_code == 201, f"Category creation failed: {cat_resp.text}"
        cat_id = cat_resp.json()["id"]

        # ---------- CREATE TASK (User A) ----------
        task_data = {"title": "Task A1", "energy_required": "low", "deadline": (datetime.now()+timedelta(days=1)).isoformat()}
        ...
        resp = await client.put(f"/tasks/{task_id}", json=update_data, headers=headers_a)
        ...
        resp = await client.put(f"/tasks/{task_id}", json={"title": "Hacked"}, headers=headers_b)
        assert resp.status_code == 403, "User B should not be able to modify User A's task"
        ...
        assert all(t["id"] != task_id for t in resp.json()), "Task still present after deletion"

        # ---------- CREATE EVENT (User B) ----------
        event_data = {"title": "Event B1", "start_time": datetime.now().isoformat(), "end_time": (datetime.now()+timedelta(hours=1)).isoformat(), "category_id": cat_id}
        ...
        # ---------- UPDATE EVENT (User B) ----------
        resp = await client.put(f"/events/{event_id}", json={"title": "Event B Updated"}, headers=headers_b)
