...
import { Task, FocusStats, FocusSession } from '../types';
...
const FocusMode: React.FC = () => {
    ...
    useEffect(() => {
        if (location.state?.taskId && tasks.length > 0) {
            setSelectedTask(String(location.state.taskId));
        }
    ...
    const loadData = async () => {
        try {
            ...
            setTasks(tasksData.filter((t: Task) => !t.is_completed));
            setStats(statsData);
    ...
    const handleStart = async () => {
        try {
            const newSession = await startFocusSession(selectedTask ? parseInt(selectedTask) : undefined);
    ...
    const handleStop = async () => {
        ...
        // Ask for feedback
        const scoreStr = prompt("¿Qué tal fue tu sesión? (1-5)");
        ...
        try {
            await stopFocusSession(session.id, score, completed);
            setSession(null);
    ...
    const handleInterrupt = async () => {
        ...
        try {
            await logInterruption(session.id, reason || undefined);
